...
Grammar::Grammar(IAllocator* _pAllocator) : m_instructions(_pAllocator, 817u), m_operandNames(_pAllocator, 791u), m_operandParameters(_pAllocator, 104u), m_operandBases(_pAllocator, 3u)
{
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0001), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0002), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0004), _pAllocator).kv.value;
		parameters.reserve(2u);
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0008), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0010), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0020), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0040), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0080), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0100), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0200), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdScope, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ImageOperands), 0x0200), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x0008), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x0010), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x0020), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x0040), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x0080), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x0100), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x10000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x20000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x40000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x80000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x100000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x200000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x400000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::LoopControl), 0x800000), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::MemoryAccess), 0x0002), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::MemoryAccess), 0x0008), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdScope, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::MemoryAccess), 0x0008), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::MemoryAccess), 0x0010), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdScope, OperandCategory::Id, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::MemoryAccess), 0x0010), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 0), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 17), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 18), _pAllocator).kv.value;
		parameters.reserve(3u);
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'x size'", Quantifier::One);
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'y size'", Quantifier::One);
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'z size'", Quantifier::One);
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 26), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Vertex count'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 30), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Vector type'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 35), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Subgroup Size'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 36), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Subgroups Per Workgroup'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 37), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "'Subgroups Per Workgroup'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 38), _pAllocator).kv.value;
		parameters.reserve(3u);
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 39), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "'Local Size Hint'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 4459), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 4460), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 4461), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 4462), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 4463), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5270), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Primitive count'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5618), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Size'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5620), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5621), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5622), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5623), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5893), _pAllocator).kv.value;
		parameters.reserve(3u);
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5894), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'max_dimensions'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5896), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'vector_width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::ExecutionMode), 5903), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'target_fmax'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 1), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Specialization Constant ID'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 6), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Array Stride'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 7), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Matrix Stride'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 11), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::BuiltIn, OperandCategory::ValueEnum, "", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 27), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdScope, OperandCategory::Id, "'Execution'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 29), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Stream Number'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 30), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Location'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 31), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Component'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 32), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Index'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 33), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Binding Point'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 34), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Descriptor Set'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 35), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Byte Offset'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 36), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'XFB Buffer Number'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 37), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'XFB Stride'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 38), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 39), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::FPRoundingMode, OperandCategory::ValueEnum, "'Floating-Point Rounding Mode'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 40), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::FPFastMathMode, OperandCategory::BitEnum, "'Fast-Math Mode'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 41), _pAllocator).kv.value;
		parameters.reserve(2u);
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 43), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Attachment Index'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 44), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Alignment'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 45), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Max Byte Offset'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 46), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "'Alignment'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 47), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::IdRef, OperandCategory::Id, "'Max Byte Offset'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5256), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Offset'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5599), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'N'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5607), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralString, OperandCategory::Literal, "'Register'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5625), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Kind'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5628), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Offset'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5634), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5635), _pAllocator).kv.value;
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5636), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralString, OperandCategory::Literal, "'User Type'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5822), _pAllocator).kv.value;
		parameters.reserve(2u);
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
		parameters.emplace_back(OperandKind::FPRoundingMode, OperandCategory::ValueEnum, "'FP Rounding Mode'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5823), _pAllocator).kv.value;
		parameters.reserve(2u);
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
		parameters.emplace_back(OperandKind::FPDenormMode, OperandCategory::ValueEnum, "'FP Denorm Mode'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5826), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralString, OperandCategory::Literal, "'Memory Type'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5827), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Banks'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5828), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Bank Width'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5829), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Maximum Copies'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5832), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Maximum Replicates'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5834), _pAllocator).kv.value;
		parameters.reserve(2u);
	...
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5835), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Bank Bits'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5836), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Force Key'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5900), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Cache Size in bytes'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5902), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Prefetcher Size in bytes'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5921), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Buffer Location ID'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 5944), _pAllocator).kv.value;
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'IO Pipe ID'", Quantifier::One);
	}
	{
		auto& parameters = m_operandParameters.emplaceUnique(Hash64(static_cast<unsigned int>(OperandKind::Decoration), 6080), _pAllocator).kv.value;
		parameters.reserve(2u);
		parameters.emplace_back(OperandKind::LiteralInteger, OperandCategory::Literal, "'Target Width'", Quantifier::One);
